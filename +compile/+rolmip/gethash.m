function [success, fileinfo] = gethash(overwrite, nobuild)
	%FINDHASH compile gethash.c file for ROLMIP
	%	Input:
	...
	filename = 'gethash';
	file		= realpath(fullfile(compile.destpath(), 'lib', 'ROLMIP', '@rolmipvar', 'private', filename));
	...
	try
		if overwrite
			...
			if false && config.EnableOpenMP
				Args(end + 1:end + 6) = {
					'LINKFLAGS=$LINKFLAGS /openmp /nodefaultlib:vcomp';
