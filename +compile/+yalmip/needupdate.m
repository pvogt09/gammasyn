function [needsupdate, changedfiles] = needupdate(report)
	%NEEDUPDATE return if YALMIP functions neeed to be recompiled
	%	Input:
	%		report:			indicator, if a report should be generated
	%	Output:
	%		needsupdate:	true, if YALMIP functions need to be recompiled
	%		changedfiles:	changed files that caused the recompilation
	if nargin < 1
		report = false;
	end
	[~, files] = compile.yalmip.all(false, true);
	...
	if report
		if any(needsupdate)
			...
			if ~isempty(changes)
				cprintf('red', 'YALMIP functions must be recompiled.\n');
				...
				cprintf('green', 'YALMIP functions do not have to be recompiled.\n');
			end
		else
			cprintf('green', 'YALMIP functions do not have to be recompiled.\n');
		end
