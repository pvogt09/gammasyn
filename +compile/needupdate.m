function [needsupdate, changedfiles] = needupdate(report)
	%NEEDUPDATE return if functions neeed to be recompiled
	%	Input:
	%		report:			indicator, if a report should be generated
	%	Output:
	%		needsupdate:	true, if functions need to be recompiled
	%		changedfiles:	changed files that caused the recompilation
	if nargin < 1
		report = false;
	end
	[~, files] = compile.all(false, true);
	...
	parfor ii = 1:size(files, 1)
		...
		if ~isempty(b{1, 2})
			...
			if isC
				cfileinfo = dir(fileinfo.c);
				ctime = cfileinfo.datenum;
				if ctime < buildtime
					needsupdate(ii) = true;
					changedfiles(ii, :) = {fileinfo, {fileinfo.build}, b{1, 1}};
					continue;
	...
	if report
		if any(needsupdate)
			...
			if ~isempty(changes)
				cprintf('red', 'Functions must be recompiled.\n');
				...
				cprintf('green', 'Functions do not have to be recompiled.\n');
			end
		else
			cprintf('green', 'Functions do not have to be recompiled.\n');
		end
