function [needsupdate, changedfiles] = needupdate(report)
	%NEEDUPDATE return if gamma area control functions neeed to be recompiled
	%	Input:
	...
	[~, files] = compile.control.design.gamma.all(false, true);
	...
	if report
		if any(needsupdate)
			...
			if ~isempty(changes)
				cprintf('red', 'Gamma Area Control functions must be recompiled.\n');
				...
				cprintf('green', 'Gamma Area Control functions do not have to be recompiled.\n');
			end
		else
			cprintf('green', 'Gamma Area Control functions do not have to be recompiled.\n');
		end
