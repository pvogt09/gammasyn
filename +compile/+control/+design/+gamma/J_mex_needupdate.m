function [needsupdate, changedfiles] = J_mex_needupdate(report)
	%NEEDUPDATE return if gamma area control objective functions neeed to be recompiled
	%	Input:
	...
	[~, files] = compile.control.design.gamma.J_mex(false, true);
	...
	if report
		if any(needsupdate)
			...
			if ~isempty(changes)
				cprintf('red', 'Gamma Area Control objective functions must be recompiled.\n');
				...
				cprintf('green', 'Gamma Area Control objective functions do not have to be recompiled.\n');
			end
		else
			cprintf('green', 'Gamma Area Control objective functions do not have to be recompiled.\n');
		end
