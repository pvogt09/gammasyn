function [success, filelist] = c_mex(overwrite, nobuild)
	%C_MEX compile constraint functions with different number of output arguments for gamma pole placement
	%	Input:
	...
	buildfiles = {
		@compile.control.design.gamma.c_mex_fun,				[];
		@compile.control.design.gamma.c_mex_grad,				[];
		@compile.control.design.gamma.c_mex_hess,				[]
	};
	if nargout >= 2
		filelist = buildfiles;
	end
	...
	wait = Progress(size(buildfiles, 1), 'Compiling Gamma Area Control constraint functions');
