classdef patternsearch < optimization.options.Options
	%PATTERNSEARCH solver options for patternsearch
	...
	methods
		function [this] = patternsearch(varargin)
			%PATTERNSEARCH create new optimization option set
			%	Input:
			%		varargin:	options to set
			%	Output:
			%		this:		instance
			this@optimization.options.Options(optimization.solver.Optimizer.PATTERNSEARCH, [
				optimization.options.OptionType.PSOPTIMSET;
				optimization.options.OptionType.STRUCT;
			], optimization.options.OptionType.PSOPTIMSET, true, [
		...
		function [information] = formatOutput(this, errorcode, time, xmin, fmin, nvars, overalliterations, overallfunevals, retries, output, alloutputs)
			...
			if outputstruct && isfield(output, 'maxconstraint') && ~isempty(output.maxconstraint)
				information.constrviolation = output.maxconstraint;
			end
			information.algorithm		= 'PATTERNSEARCH';
			...
			if outputstruct && isfield(output, 'maxconstraint') && ~isempty(output.maxconstraint)
				information.information.constrviol		= output.maxconstraint;
			end
			if outputstruct && isfield(output, 'message')
