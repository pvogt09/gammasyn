classdef fminsearch < optimization.options.Options
	...
	methods
		function [this] = fminsearch(varargin)
			...
			this@optimization.options.Options(optimization.solver.Optimizer.FMINSEARCH, [
				optimization.options.OptionType.OPTIMSET;
				optimization.options.OptionType.STRUCT
			], preferredtype, true, optimization.options.ProblemType.UNCONSTRAINED, optimization.options.ProblemType.UNCONSTRAINED);
			if nargin >= 1
		...
		function [information] = formatOutput(this, errorcode, time, xmin, fmin, nvars, overalliterations, overallfunevals, retries, output, alloutputs)
			...
			if outputstruct && isfield(output, 'algorithm')
				...
				information.algorithm		= 'FMINSEARCH';
			end
