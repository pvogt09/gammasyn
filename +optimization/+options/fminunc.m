classdef fminunc < optimization.options.Options
	%FMINUNC solver options for fminunc
	...
	methods
		function [this] = fminunc(varargin)
			%FMINUNC create new optimization option set
			%	Input:
			%		varargin:	options to set
			%	Output:
			%		this:		instance
			if matlab.Version.CURRENT >= matlab.Version.R2013A
				preferredtype = optimization.options.OptionType.OPTIMOPTIONS;
				supportedtype = [
					optimization.options.OptionType.OPTIMOPTIONS;
					optimization.options.OptionType.OPTIMSET;
					optimization.options.OptionType.STRUCT
				];
			else
				preferredtype = optimization.options.OptionType.OPTIMSET;
				supportedtype = [
					optimization.options.OptionType.OPTIMSET;
					optimization.options.OptionType.STRUCT
				];
			end
			this@optimization.options.Options(optimization.solver.Optimizer.FMINUNC, supportedtype, preferredtype, true, optimization.options.ProblemType.UNCONSTRAINED, optimization.options.ProblemType.UNCONSTRAINED);
			if nargin >= 1
