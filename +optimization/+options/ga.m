classdef ga < optimization.options.Options
	...
	methods
		function [this] = ga(varargin)
			...
			%	Output:
			%		this:		instance
			this@optimization.options.Options(optimization.solver.Optimizer.GA, [
				optimization.options.OptionType.GAOPTIMSET;
				optimization.options.OptionType.STRUCT
			], optimization.options.OptionType.GAOPTIMSET, true, [
		...
		function [information] = formatOutput(this, errorcode, time, xmin, fmin, nvars, overalliterations, overallfunevals, retries, output, alloutputs)
			...
			if outputstruct && isfield(output, 'generations')
				information.iterations		= output.generations;
			end
			...
			if outputstruct && isfield(output, 'maxconstraint')
				information.constrviolation = output.maxconstraint;
			end
			information.algorithm		= 'GA';
			...
			if outputstruct && isfield(output, 'generations')
				information.information.iterations		= output.generations;
			end
			if outputstruct && isfield(output, 'funcCount')
