classdef solvopt < optimization.options.Options
	%SOLVOPT solver options for solvopt
	...
	methods(Static=true)
		function [this] = fromDCM(value, ~)
			...
			%	Output:
			%		this:	instance
			this = optimization.options.ksopt();
		...
		function [success] = registerDCMHandler()
			...
			success = export.parameter.DCMHandler.instance.registerHandler(?optimization.options.ksopt, ?optimization.options.ksopt);
		end
	end
	...
	methods
		function [this] = solvopt(varargin)
			%SOLVOPT create new optimization option set
			%	Input:
			...
			this@optimization.options.Options(optimization.solver.Optimizer.SOLVOPT, supportedtype, preferredtype, false, [
