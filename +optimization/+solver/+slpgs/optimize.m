function [x, fval, exitflag, output, lambda, grad, hessian] = optimize(fun, x_0, A, b, Aeq, beq, lb, ub, nonlcon, options, varargin)
	%OPTIMIZE call slpgs with constrained algorithm to solve optimization problem
	%	Input:
	...
	defaultsolver = optimization.solver.Optimizer.SLPGS;
	...
	if ~isa(options, 'optimization.options.Options')
		if isstruct(options)
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.SLPGS, options);
	...
	if ~isa(options, 'optimization.options.slpgs')
		options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.SLPGS, options);
	end
	try
		if ~isempty(nonlcon)
	...
	solveroptions.algorithm = 1;
	solveroptions.sp_solver = solveroptions.SubproblemAlgorithm;
	if isempty(solveroptions.sp_solver)
		solveroptions.sp_solver = 'linprog';
	end
	if ~strcmpi(solveroptions.sp_solver, 'linprog')
		error('optimization:solver:slqpgs:input', 'Algorithm slqpgs only supports subproblem algorithm ''linprog''.');
	end
	...
	while retry < max(1, options.Retries)
		...
		if useparallel
			parfor ii = 1:initialvalues
				...
				% clear persistent variables to initialize correctly on all workers
	...
	if isempty(fvalidx)
		error('optimization:solver:slqpgs:solution', 'No solution could be found for any initial value.');
	end
	idx = feasidx(fvalidx);
	if isempty(idx)
		error('optimization:solver:slqpgs:solution', 'No solution could be found for any initial value.');
