function [x, fval, exitflag, output, lambda, grad, hessian] = optimize(fun, x_0, A, b, Aeq, beq, lb, ub, ~, options, varargin)
	%OPTIMIZE call fminunc to solve optimization problem
	%	Input:
	...
	if isempty(fun)
		error('optimization:solver:fminunc:input', 'Objective function must not be empty.');
	end
	if ischar(fun)
		fun = str2func(fun);
	end
	if ~isfunctionhandle(fun)
		error('optimization:solver:fminunc:input', 'Objective function must be a function handle.');
	end
	if isempty(x_0)
		error('optimization:solver:fminunc:input', 'Initial point must not be empty.');
	end
	if isa(x_0, 'AbstractStartPointSet')
		x_0 = x_0.list().';
	end
	if ~isreal(x_0) || any(isinf(x_0(:))) || any(isnan(x_0(:)))
		error('optimization:solver:fminunc:input', 'Initial point must be a finite real value.');
	end
	if ~ismatrix(x_0)
		error('optimization:solver:fminunc:input', 'Initial point must be a column vector or a matrix of column vectors.');
	end
	%if size(x_0, 2) > 1
	%	error('optimization:solver:fminunc:input', 'Initial point must be a column vector.');
	%end
	usedefaultoption = false;
	defaultsolver = optimization.solver.Optimizer.FMINUNC;
	...
	if ~isreal(A) || any(isnan(A(:)))
		error('optimization:solver:fminunc:input', 'Inequality matrix must be real.');
	end
	if ~ismatrix(A)
		error('optimization:solver:fminunc:input', 'Inequality matrix must be a matrix.');
	end
	if size(A, 2) ~= size(x_0, 1)
		error('optimization:solver:fminunc:input', 'Inequality matrix must have %d columns, not %d.', size(x_0, 1), size(A, 2));
	end
	if ~isreal(b) || any(isnan(b(:)))
		error('optimization:solver:fminunc:input', 'Inequality bounds must be real.');
	end
	if ~ismatrix(b)
		error('optimization:solver:fminunc:input', 'Inequality bounds must be a vector.');
	end
	if size(b, 1) ~= size(A, 1)
		error('optimization:solver:fminunc:input', 'Inequality bounds must have %d rows, not %d.', size(A, 1), size(b, 1));
	end
	if ~isreal(Aeq) || any(isnan(Aeq(:)))
		error('optimization:solver:fminunc:input', 'Equality matrix must be real.');
	end
	if ~ismatrix(Aeq)
		error('optimization:solver:fminunc:input', 'Equality matrix must be a matrix.');
	end
	if size(Aeq, 2) ~= size(x_0, 1)
		error('optimization:solver:fminunc:input', 'Equality matrix must have %d columns, not %d.', size(x_0, 1), size(Aeq, 2));
	end
	if ~isreal(beq) || any(isnan(beq(:)))
		error('optimization:solver:fminunc:input', 'Equality bounds must be real.');
	end
	if ~ismatrix(beq)
		error('optimization:solver:fminunc:input', 'Equality bounds must be a vector.');
	end
	if size(beq, 1) ~= size(Aeq, 1)
		error('optimization:solver:fminunc:input', 'Equality bounds must have %d rows, not %d.', size(Aeq, 1), size(beq, 1));
	end
	if ~isreal(lb) || any(isnan(lb(:)))
		error('optimization:solver:fminunc:input', 'Lower bounds must be real.');
	end
	if ~ismatrix(lb)
		error('optimization:solver:fminunc:input', 'Lower bounds must be a vector.');
	end
	if size(lb, 1) ~= size(x_0, 1)
		error('optimization:solver:fminunc:input', 'Lower bounds must have %d rows, not %d.', size(x_0, 1), size(lb, 1));
	end
	if ~isreal(ub) || any(isnan(ub(:)))
		error('optimization:solver:fminunc:input', 'Upper bounds must be real.');
	end
	if ~ismatrix(ub)
		error('optimization:solver:fminunc:input', 'Upper bounds must be a vector.');
	end
	if size(ub, 1) ~= size(x_0, 1)
		error('optimization:solver:fminunc:input', 'Upper bounds must have %d rows, not %d.', size(x_0, 1), size(ub, 1));
	end
	if ~isa(options, 'optimization.options.Options')
		if isstruct(options)
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.FMINUNC, options);
		elseif isa(options, 'optim.options.SolverOptions')
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.FMINUNC, options);
		else
			error('optimization:solver:fminunc:input', 'Options must be of type optimization.options.Options or optimoptions or optimset.');
		end
	end
	if ~isa(options, 'optimization.options.fminunc')
		options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.FMINUNC, options);
	end
	...
	if needshessian && ~hashessian
		error('optimization:solver:fminunc:input', 'User supplied objective function does not return hessian, but calculation of hessian matrix is requested.');
	end
	...
	while retry < max(1, options.Retries)
		...
		if useparallel
			parfor ii = 1:initialvalues
				try
					if nargout7
						[x_opt(:, ii), f_opt(1, ii), exitflag(1, ii), output{1, ii}, grad{1, ii}, hessian{1, ii}] = fminunc(Jfun, x_0(:, ii), solveroptions, varargin{:}); %#ok<PFBNS> varargin is a constant broadcast variable
					elseif nargout6
						[x_opt(:, ii), f_opt(1, ii), exitflag(1, ii), output{1, ii}, grad{1, ii}, hessian{1, ii}] = fminunc(Jfun, x_0(:, ii), solveroptions, varargin{:});
					elseif nargout4
						[x_opt(:, ii), f_opt(1, ii), exitflag(1, ii), output{1, ii}] = fminunc(Jfun, x_0(:, ii), solveroptions, varargin{:});
					elseif nargout3
						[x_opt(:, ii), f_opt(1, ii), exitflag(1, ii)] = fminunc(Jfun, x_0(:, ii), solveroptions, varargin{:});
					else
						[x_opt(:, ii), f_opt(1, ii)] = fminunc(Jfun, x_0(:, ii), solveroptions, varargin{:});
					end
				catch e
					rethrow(e);
				end
