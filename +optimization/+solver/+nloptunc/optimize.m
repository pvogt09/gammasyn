function [x, fval, exitflag, output, lambda, grad, hessian] = optimize(fun, x_0, A, b, Aeq, beq, lb, ub, nonlcon, options, varargin)
	...
	if isempty(fun)
		error('optimization:solver:nloptunc:input', 'Objective function must not be empty.');
	end
	if ischar(fun)
		fun = str2func(fun);
	end
	if ~isfunctionhandle(fun)
		error('optimization:solver:nloptunc:input', 'Objective function must be a function handle.');
	end
	if isempty(x_0)
		error('optimization:solver:nloptunc:input', 'Initial point must not be empty.');
	end
	if isa(x_0, 'AbstractStartPointSet')
		x_0 = x_0.list().';
	end
	if ~isreal(x_0) || any(isinf(x_0(:))) || any(isnan(x_0(:)))
		error('optimization:solver:nloptunc:input', 'Initial point must be a finite real value.');
	end
	if ~ismatrix(x_0)
		error('optimization:solver:nloptunc:input', 'Initial point must be a column vector or a matrix of column vectors.');
	end
	%if size(x_0, 2) > 1
	%	error('optimization:solver:nloptunc:input', 'Initial point must be a column vector.');
	%end
	usedefaultoption = false;
	defaultsolver = optimization.solver.Optimizer.NLOPTUNC;
	...
	if ~isreal(A) || any(isnan(A(:)))
		error('optimization:solver:nloptunc:input', 'Inequality matrix must be real.');
	end
	if ~ismatrix(A)
		error('optimization:solver:nloptunc:input', 'Inequality matrix must be a matrix.');
	end
	if size(A, 2) ~= size(x_0, 1)
		error('optimization:solver:nloptunc:input', 'Inequality matrix must have %d columns, not %d.', size(x_0, 1), size(A, 2));
	end
	if ~isreal(b) || any(isnan(b(:)))
		error('optimization:solver:nloptunc:input', 'Inequality bounds must be real.');
	end
	if ~ismatrix(b)
		error('optimization:solver:nloptunc:input', 'Inequality bounds must be a vector.');
	end
	if size(b, 1) ~= size(A, 1)
		error('optimization:solver:nloptunc:input', 'Inequality bounds must have %d rows, not %d.', size(A, 1), size(b, 1));
	end
	if ~isreal(Aeq) || any(isnan(Aeq(:)))
		error('optimization:solver:nloptunc:input', 'Equality matrix must be real.');
	end
	if ~ismatrix(Aeq)
		error('optimization:solver:nloptunc:input', 'Equality matrix must be a matrix.');
	end
	if size(Aeq, 2) ~= size(x_0, 1)
		error('optimization:solver:nloptunc:input', 'Equality matrix must have %d columns, not %d.', size(x_0, 1), size(Aeq, 2));
	end
	if ~isreal(beq) || any(isnan(beq(:)))
		error('optimization:solver:nloptunc:input', 'Equality bounds must be real.');
	end
	if ~ismatrix(beq)
		error('optimization:solver:nloptunc:input', 'Equality bounds must be a vector.');
	end
	if size(beq, 1) ~= size(Aeq, 1)
		error('optimization:solver:nloptunc:input', 'Equality bounds must have %d rows, not %d.', size(Aeq, 1), size(beq, 1));
	end
	if ~isreal(lb) || any(isnan(lb(:)))
		error('optimization:solver:nloptunc:input', 'Lower bounds must be real.');
	end
	if ~ismatrix(lb)
		error('optimization:solver:nloptunc:input', 'Lower bounds must be a vector.');
	end
	if size(lb, 1) ~= size(x_0, 1)
		error('optimization:solver:nloptunc:input', 'Lower bounds must have %d rows, not %d.', size(x_0, 1), size(lb, 1));
	end
	if ~isreal(ub) || any(isnan(ub(:)))
		error('optimization:solver:nloptunc:input', 'Upper bounds must be real.');
	end
	if ~ismatrix(ub)
		error('optimization:solver:nloptunc:input', 'Upper bounds must be a vector.');
	end
	if size(ub, 1) ~= size(x_0, 1)
		error('optimization:solver:nloptunc:input', 'Upper bounds must have %d rows, not %d.', size(x_0, 1), size(ub, 1));
	end
	if nargin >= 9
		if ~isempty(nonlcon)
			if ischar(nonlcon)
			...
			if ~isfunctionhandle(nonlcon)
				error('optimization:solver:nloptunc:input', 'Constraint function must be a function handle.');
			end
		end
	else
		nonlcon = [];
	...
	if ~isa(options, 'optimization.options.Options')
		if isstruct(options)
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTUNC, options);
		elseif isa(options, 'optim.options.SolverOptions')
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTUNC, options);
		else
			error('optimization:solver:nloptunc:input', 'Options must be of type optimization.options.Options or optimoptions or optimset.');
		end
	end
	if ~isa(options, 'optimization.options.nloptunc')
		options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTUNC, options);
	end
	try
		if ~isempty(nonlcon)
	...
	if isnan(algorithm)
		error('optimization:solver:nloptunc:input', 'Current algorithm is not supported.');
	end
	if needsgradient && ~gradient
		error('optimization:solver:nloptunc:input', 'Current algorithm needs gradient information.');
	end
	if needsbounds
		if any(isinf(lb(:))) || any(isinf(ub(:)))
			...
			if any(isinf(lb(:))) || any(isinf(ub(:))) || any(lb > ub)
				error('optimization:solver:nloptunc:input', 'Bounds must be specified for the current algorithm.');
			else
				warning('optimization:solver:nloptunc:input', 'Bounds must be specified for the current algorithm. Using [x_0 - 10*x_0, x_0 + 10*x_0] as bound.');
			end
		end
	end
	if isglobal
		error('optimization:solver:nloptunc:input', 'nloptunc is for unconstrained problems, for global optimization use nloptuncglobal instead.');
	end
	...
	if isempty(idx)
		error('optimization:solver:nloptunc:solution', 'No solution could be found for any initial value.');
	end
	x = x_opt(:, idx);
	if nargout3
...
function [J] = objective_dispatcher(fun, gradient)
	%OBJECTIVE_DISPATCHER wrapper function for nloptunc to convert the optimization variable to a column vector in case optimization variable is a row vector
	%	Input:
