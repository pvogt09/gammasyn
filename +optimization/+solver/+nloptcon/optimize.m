function [x, fval, exitflag, output, lambda, grad, hessian] = optimize(fun, x_0, A, b, Aeq, beq, lb, ub, nonlcon, options, varargin)
	%OPTIMIZE call nlopt with constrained algorithm to solve optimization problem
	%	Input:
	...
	if isempty(fun)
		error('optimization:solver:nloptcon:input', 'Objective function must not be empty.');
	end
	if ischar(fun)
		fun = str2func(fun);
	end
	if ~isfunctionhandle(fun)
		error('optimization:solver:nloptcon:input', 'Objective function must be a function handle.');
	end
	if isempty(x_0)
		error('optimization:solver:nloptcon:input', 'Initial point must not be empty.');
	end
	if isa(x_0, 'AbstractStartPointSet')
		x_0 = x_0.list().';
	end
	if ~isreal(x_0) || any(isinf(x_0(:))) || any(isnan(x_0(:)))
		error('optimization:solver:nloptcon:input', 'Initial point must be a finite real value.');
	end
	if ~ismatrix(x_0)
		error('optimization:solver:nloptcon:input', 'Initial point must be a column vector or a matrix of column vectors.');
	end
	%if size(x_0, 2) > 1
	%	error('optimization:solver:nloptcon:input', 'Initial point must be a column vector.');
	%end
	usedefaultoption = false;
	defaultsolver = optimization.solver.Optimizer.NLOPTCON;
	...
	if ~isreal(A) || any(isnan(A(:)))
		error('optimization:solver:nloptcon:input', 'Inequality matrix must be real.');
	end
	if ~ismatrix(A)
		error('optimization:solver:nloptcon:input', 'Inequality matrix must be a matrix.');
	end
	if size(A, 2) ~= size(x_0, 1)
		error('optimization:solver:nloptcon:input', 'Inequality matrix must have %d columns, not %d.', size(x_0, 1), size(A, 2));
	end
	if ~isreal(b) || any(isnan(b(:)))
		error('optimization:solver:nloptcon:input', 'Inequality bounds must be real.');
	end
	if ~ismatrix(b)
		error('optimization:solver:nloptcon:input', 'Inequality bounds must be a vector.');
	end
	if size(b, 1) ~= size(A, 1)
		error('optimization:solver:nloptcon:input', 'Inequality bounds must have %d rows, not %d.', size(A, 1), size(b, 1));
	end
	if ~isreal(Aeq) || any(isnan(Aeq(:)))
		error('optimization:solver:nloptcon:input', 'Equality matrix must be real.');
	end
	if ~ismatrix(Aeq)
		error('optimization:solver:nloptcon:input', 'Equality matrix must be a matrix.');
	end
	if size(Aeq, 2) ~= size(x_0, 1)
		error('optimization:solver:nloptcon:input', 'Equality matrix must have %d columns, not %d.', size(x_0, 1), size(Aeq, 2));
	end
	if ~isreal(beq) || any(isnan(beq(:)))
		error('optimization:solver:nloptcon:input', 'Equality bounds must be real.');
	end
	if ~ismatrix(beq)
		error('optimization:solver:nloptcon:input', 'Equality bounds must be a vector.');
	end
	if size(beq, 1) ~= size(Aeq, 1)
		error('optimization:solver:nloptcon:input', 'Equality bounds must have %d rows, not %d.', size(Aeq, 1), size(beq, 1));
	end
	if ~isreal(lb) || any(isnan(lb(:)))
		error('optimization:solver:nloptcon:input', 'Lower bounds must be real.');
	end
	if ~ismatrix(lb)
		error('optimization:solver:nloptcon:input', 'Lower bounds must be a vector.');
	end
	if size(lb, 1) ~= size(x_0, 1)
		error('optimization:solver:nloptcon:input', 'Lower bounds must have %d rows, not %d.', size(x_0, 1), size(lb, 1));
	end
	if ~isreal(ub) || any(isnan(ub(:)))
		error('optimization:solver:nloptcon:input', 'Upper bounds must be real.');
	end
	if ~ismatrix(ub)
		error('optimization:solver:nloptcon:input', 'Upper bounds must be a vector.');
	end
	if size(ub, 1) ~= size(x_0, 1)
		error('optimization:solver:nloptcon:input', 'Upper bounds must have %d rows, not %d.', size(x_0, 1), size(ub, 1));
	end
	if nargin >= 9
		if ~isempty(nonlcon)
			if ischar(nonlcon)
			...
			if ~isfunctionhandle(nonlcon)
				error('optimization:solver:nloptcon:input', 'Constraint function must be a function handle.');
			end
		end
	else
		nonlcon = [];
	...
	if ~isa(options, 'optimization.options.Options')
		if isstruct(options)
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTCON, options);
		elseif isa(options, 'optim.options.SolverOptions')
			options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTCON, options);
		else
			error('optimization:solver:nloptcon:input', 'Options must be of type optimization.options.Options or optimoptions or optimset.');
		end
	end
	if ~isa(options, 'optimization.options.nloptcon')
		options = optimization.options.OptionFactory.instance.options(optimization.solver.Optimizer.NLOPTCON, options);
	end
	try
		if ~isempty(nonlcon)
	...
	if isnan(algorithm)
		error('optimization:solver:nloptcon:input', 'Current algorithm is not supported.');
	end
	if needsgradient && ~gradient
		error('optimization:solver:nloptcon:input', 'Current algorithm needs gradient information.');
	end
	if needsbounds
		if any(isinf(lb(:))) || any(isinf(ub(:)))
			...
			if any(isinf(lb(:))) || any(isinf(ub(:))) || any(lb > ub)
				error('optimization:solver:nloptcon:input', 'Bounds must be specified for the current algorithm.');
			else
				warning('optimization:solver:nloptcon:input', 'Bounds must be specified for the current algorithm. Using [x_0 - 10*x_0, x_0 + 10*x_0] as bound.');
			end
		end
	end
	if isglobal
		error('optimization:solver:nloptcon:input', 'nloptcon is for unconstrained problems, for global optimization use nloptconglobal instead.');
	end
	...
	if ~nloptinterface && ~optiinterface
		error('optimization:solver:nloptcon:input', 'No function interface for nlopt could be found.');
	end
	if optiinterface
		nloptinterface = false;
	end
	...
	if nloptinterface
		...
		if size(A, 1) + size(c, 1) > 0
			solveroptions.fc = constraint_dispatcher_ineq(nonlcon, A, b);
		end
		if size(Aeq, 1) + size(ceq, 1) > 0
			solveroptions.h = constraint_dispatcher_eq(nonlcon, Aeq, beq);
		end
	...
	if isempty(fvalidx)
		error('optimization:solver:nloptcon:solution', 'No solution could be found for any initial value.');
	end
	idx = feasidx(fvalidx);
	if isempty(idx)
		error('optimization:solver:nloptcon:solution', 'No solution could be found for any initial value.');
...
function [J] = objective_dispatcher(fun, gradient)
	%OBJECTIVE_DISPATCHER wrapper function for nloptcon to convert the optimization variable to a column vector in case optimization variable is a row vector
	%	Input:
...
function [cfun] = constraint_dispatcher(fun, A, b, Aeq, beq, gradient)
	%CONSTRAINT_DISPATCHER wrapper function for nloptcon to convert the optimization variable to a column vector in case optimization variable is a row vector and split equality from inequality constraints
	%	Input:
		...
		if isempty(fun)
			if isempty(A) && isempty(b) && isempty(Aeq) && isempty(beq)
				...
				if xor(isempty(A), isempty(b)) || xor(isempty(Aeq), isempty(beq))
					error('optimization:solver:nloptcon:input', 'A and b must be both empty or not empty.');
				end
...
function [c] = constraint_dispatcher_ineq(fun, A, b)
	%CONSTRAINT_DISPATCHER_INEQ wrapper function for nloptcon to convert the optimization variable to a column vector in case optimization variable is a row vector and split equality from inequality constraints
	%	Input:
	%		fun:	constraint function handle
	%		A:		inequality constraint matrix
	%		b:		inequality constraint bounds
...
function [ceq] = constraint_dispatcher_eq(fun, Aeq, beq)
	%CONSTRAINT_DISPATCHER_EQ wrapper function for nloptcon to convert the optimization variable to a column vector in case optimization variable is a row vector and split equality from inequality constraints
	%	Input:
	%		fun:		constraint function handle
	%		Aeq:		equality constraint matrix
	%		beq:		equality constraint bounds
	...
	if isempty(fun)
		if isempty(Aeq) && isempty(beq)
			...
			if xor(isempty(Aeq), isempty(beq))
				error('optimization:solver:nloptcon:input', 'Aeq and beq must be both empty or not empty.');
			end
