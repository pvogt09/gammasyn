function [P_opt, ew_opt, st_opt, ews, w1, w2, npq, index] = eigenvals(ew, A, P, npq, interactive)
	...
	if check && check < npq(4)
		h_ew1 = zeros(check, 1);	% Hilfsvariable f�r die gewichteten EW
		h_ew2 = zeros(npq(4) - check, 1);	% Hilfsvariable f�r die gewichteten EW
		h_w1 = zeros(check, 1);	% Hilfsvariable f�r die Gewichtungsfaktoren
		h_P1 = zeros(npq(2) + 1, check);	% Hilfsvariable f�r die Parametervektoren
		h_P2 = zeros(npq(2) + 1, npq(4) - check);	% Hilfsvariable f�r die Parametervektoren
	...
	% Teil III: St�tzstellen abfragen
	if npq(5)
		st_opt = [];
		d = npq(5);
		fak = 40;
		...
		% Teil IV: Ausgabe der St�tzstellen plus Vorschlag f�r Gewichtungsfaktor
	...
	% falls eine St�tzstelle mit 0 gewichtet ist, St�tzstelle aus st_opt eliminieren
	check = sum(sign(w2));
	if check && check < npq(5)
		st_1 = zeros(check, 1);	% Hilfsvariable f�r die gewichteten St�tzstellen
		h_w2 = zeros(check, 1);	% Hilfsvariable f�r die Gewichtungsfaktoren
		jj = 1;
		for ii = 1:npq(5)
			if w2(ii) > 0
				st_1(jj) = st_opt(ii);
