function [t, x_new, f_new, g_new, funEvals] = ArmijoBacktrackMinMax(x, t, d, f, fr, g, gtd, c1, LS_interp, LS_multi, progTol, debug, doPlot, saveHessianComp, funObj, varargin)
	...
	if nargout == 6
		if nargout(funObj) >= 3
			...
			error('PPPBox:derivative:input', 'Objective function does not supply Hessian');
		end
	else
		if nargout(funObj) >= 2
			...
			[~, g_new] = autoGradMinMax(x + t*d, numDiffType, funObj, varargin{:});
		end
	end
	funEvals = 1;
