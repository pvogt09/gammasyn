classdef gammasyn_decouplingcontrol_Test_Class
	...
	methods
		...
		function this = amend_R_fixed_bounds(this, idx_RKF, idx_Ab, value, type)
			...
			if ~strcmp(type, 'fixed') && ~strcmp(type, 'bounds')
				error('Wrong identifier. type must be ''fixed'' or ''bounds''.');
			end
			type_string = ['R_', type];
		...
		function [systems, areafun, weights, R_fixed, R_0, solveroptions, objectiveoptions, R_bounds, R_nonlin, number_passed_parameters] = implement_testcase(this, testcase_struct)
			...
			for ii = 1:size(testcase_struct.R_fixed, 1)
				this = this.amend_R_fixed_bounds(testcase_struct.R_fixed{ii}{1}, testcase_struct.R_fixed{ii}{2}, testcase_struct.R_fixed{ii}{3}, 'fixed');
			end
			for ii = 1:size(testcase_struct.R_0, 1)
				this = this.amend_R_0(testcase_struct.R_0{ii}{1}, testcase_struct.R_0{ii}{2}, testcase_struct.R_0{ii}{3});
			end
			...
			for ii = 1:size(testcase_struct.R_bounds, 1)
				this = this.amend_R_fixed_bounds(testcase_struct.R_bounds{ii}{1}, testcase_struct.R_bounds{ii}{2}, testcase_struct.R_bounds{ii}{3}, 'bounds');
			end
			for ii = 1:size(testcase_struct.R_nonlin, 1)
				this = this.amend_R_nonlin(testcase_struct.R_nonlin{ii, 1});
			end
			for ii = 1:size(testcase_struct.number_passed_parameters, 1)
				this = this.amend_number_passed_parameters(testcase_struct.number_passed_parameters{ii, 1});
			end
			for ii = 1:size(testcase_struct.areafun, 1)
				this = this.amend_areafun(testcase_struct.areafun{ii, 1}, testcase_struct.areafun{ii, 2});
			end
