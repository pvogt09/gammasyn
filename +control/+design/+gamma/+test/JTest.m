function [pass] = JTest(silent)
	%JTEST test cases for checking objective function for correct argument handling
	%	Input:
	...
	hascompiled = any(usecompiled) && true && all(~compile.control.design.gamma.J_mex_needupdate());
	...
	if ~silent
		wait = Progress(N, 'Objective function test', true, 1000);
	end
	M = 1;
																	...
																	for zz = 1:size(descriptor, 2)
																		...
																		if usecompiled(1, uu)
																			if ~configuration.control.design.gamma.hascompiled()
																				test.TestSuite.assert(false, 'control:gammasyn:test', 'Objective function has not been compiled.');
																			end
																			if ~hascompiled
																				test.TestSuite.assert(false, 'control:gammasyn:test', 'Objective function must be recompiled.');
																			end
																		end
																		...
																		hashessian = GammaJType.hashessian(objective);
																		...
																		Jfun = gethandle(compiled, system, weight_loose, areafun_loose, dimensions_loose, objectiveoptions);
																		R = cell(length(R_0), 1);
																		...
																		for jjj = 1:size(combinations, 1)
																			c = combinations(jjj, :);
																			for iii = 1:size(c{1}, 3)
																				if isempty(c{2}) && isempty(c{3})
																					x = feval(checkinitialR, c{1}(:, :, iii), dimensions_loose);
																				elseif isempty(c{2})
																					x = feval(checkinitialR, {c{1}(:, :, iii), [], c{3}(:, :, iii)}, dimensions_loose);
																				elseif isempty(c{3})
																					x = feval(checkinitialR, {c{1}(:, :, iii), c{2}(:, :, iii)}, dimensions_loose);
																				else
																					x = feval(checkinitialR, {c{1}(:, :, iii), c{2}(:, :, iii), c{3}(:, :, iii)}, dimensions_loose);
																				end
																				...
																				if hashessian
																					...
																					test.TestSuite.assertSameSize(gradJ, x, 'control:gamma:arguments:test', 'Objective function gradient value must have same dimension as x.');
																					test.TestSuite.assert(~any(isnan(gradJ(:))), 'control:gamma:arguments:test', 'Objective function gradient value must not be NaN.');
																					test.TestSuite.assertSameSize(hessJ, x*x', 'control:gamma:arguments:test', 'Objective function hessian value must have same dimension as x.');
																					test.TestSuite.assert(~any(isnan(hessJ(:))), 'control:gamma:arguments:test', 'Objective function hessian value must not be NaN.');
																				end
																			end
																		end
																		if ~silent
...
function [J] = gethandle(usecompiled, system, weight, areafun, dimensions, options)
	persistent J_mex_private;
	if isempty(J_mex_private)
		J_mex_private = get_private_function(realpath(fullfile(mfilename('fullpath'), '..', '..', 'private')), 'J_mex');
	end
	J_mex = J_mex_private;
	function [J, gradJ, hessianJ] = Jfun(x)
		x = reshape(x, [], 1);
		if nargout >= 3
	...
	function [J, gradJ, hessianJ] = Jfun_mex(x)
		x = reshape(x, [], 1);
		if nargout >= 3
