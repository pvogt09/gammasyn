function [objective_options_strict, objective_options_loose, solution_strategy, errorhandler] = checkobjectiveoptions(objectiveoptions, x_0, number_states, system, solveroptions, dimensions_strict, dimensions_loose, areafun_strict, areafun_loose, weight_strict, weight_loose)
	...
	if isempty(objectiveoptions)
		...
		if ~isstruct(objectiveoptions)
			...
			if ~isfield(objectiveoptions, 'couplingcontrol')
				objectiveoptions.couplingcontrol = coupling_prototype;
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'couplingconditions')
				objectiveoptions.couplingcontrol.couplingconditions = coupling_prototype.couplingconditions;
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'couplingstrategy')
				% use default strategy in case coupling controller design is requested
				objectiveoptions.couplingcontrol.couplingstrategy = [];
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'sortingstrategy_coupling')
			...
			if ~isfield(objectiveoptions.couplingcontrol, 'tolerance_prefilter')
				objectiveoptions.couplingcontrol.tolerance_prefilter = coupling_prototype.tolerance_prefilter;
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'solvesymbolic')
				objectiveoptions.couplingcontrol.solvesymbolic = coupling_prototype.solvesymbolic;
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'round_equations_to_digits')
				objectiveoptions.couplingcontrol.round_equations_to_digits = coupling_prototype.round_equations_to_digits;
			end
			if ~isfield(objectiveoptions.couplingcontrol, 'allowoutputcoupling')
				objectiveoptions.couplingcontrol.allowoutputcoupling = coupling_prototype.allowoutputcoupling;
			end
	...
	if objectiveoptions.couplingcontrol.couplingstrategy ~= GammaCouplingStrategy.NONE
		if dimensions_strict.references < objectiveoptions.couplingcontrol.couplingconditions
			error('control:design:gamma:input', 'Number of coupling conditions must not exceed number of references.');
		end
	end
	preventNaN = NaN;
	...
	objective_options_strict = struct(...
		...
		'couplingcontrol',		struct(...
			'couplingconditions',			uint32(objectiveoptions.couplingcontrol.couplingconditions),...
			'couplingstrategy',				objectiveoptions.couplingcontrol.couplingstrategy,...
			'sortingstrategy_coupling',		objectiveoptions.couplingcontrol.sortingstrategy_coupling,...
			'weight_coupling',				double(objectiveoptions.couplingcontrol.weight_coupling),...
			'weight_prefilter',				double(objectiveoptions.couplingcontrol.weight_prefilter),...
			'tolerance_coupling',			double(objectiveoptions.couplingcontrol.tolerance_coupling),...
			'tolerance_prefilter',			double(objectiveoptions.couplingcontrol.tolerance_prefilter),...
			'solvesymbolic',				logical(objectiveoptions.couplingcontrol.solvesymbolic),...
			'round_equations_to_digits',	double(objectiveoptions.couplingcontrol.round_equations_to_digits),...
			'allowoutputcoupling',			logical(objectiveoptions.couplingcontrol.allowoutputcoupling)...
		),...
		'objective',			struct(...
