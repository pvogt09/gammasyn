function [objective_options_strict, objective_options_loose, solution_strategy, errorhandler] = checkobjectiveoptions(objectiveoptions, x_0, number_states, system, solveroptions, dimensions_strict, dimensions_loose, areafun_strict, areafun_loose, weight_strict, weight_loose)
	...
	decoupling_prototype = control.design.gamma.objectiveoptions_prototype_decoupling(dimensions_strict.references);
	if isempty(objectiveoptions)
		...
		if ~isstruct(objectiveoptions)
			...
			if ~isfield(objectiveoptions, 'decouplingcontrol')
				objectiveoptions.decouplingcontrol = decoupling_prototype;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'tf_structure')
				objectiveoptions.decouplingcontrol.tf_structure = decoupling_prototype.tf_structure;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'decouplingstrategy')
				% use default strategy in case decoupling controller design is requested
				objectiveoptions.decouplingcontrol.decouplingstrategy = [];
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'sortingstrategy_decoupling')
				% use default sorting strategy for decoupling conditions
				objectiveoptions.decouplingcontrol.sortingstrategy_decoupling = [];
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'weight_decoupling')
				objectiveoptions.decouplingcontrol.weight_decoupling = decoupling_prototype.weight_decoupling;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'weight_prefilter')
				objectiveoptions.decouplingcontrol.weight_prefilter = decoupling_prototype.weight_prefilter;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'tolerance_decoupling')
				objectiveoptions.decouplingcontrol.tolerance_decoupling = decoupling_prototype.tolerance_decoupling;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'tolerance_prefilter')
				objectiveoptions.decouplingcontrol.tolerance_prefilter = decoupling_prototype.tolerance_prefilter;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'solvesymbolic')
				objectiveoptions.decouplingcontrol.solvesymbolic = decoupling_prototype.solvesymbolic;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'round_equations_to_digits')
				objectiveoptions.decouplingcontrol.round_equations_to_digits = decoupling_prototype.round_equations_to_digits;
			end
			if ~isfield(objectiveoptions.decouplingcontrol, 'allowoutputdecoupling')
				objectiveoptions.decouplingcontrol.allowoutputdecoupling = decoupling_prototype.allowoutputdecoupling;
			end
	...
	if isfield(objectiveoptions, 'decouplingcontrol')
		objectiveoptions.decouplingcontrol = checkobjectiveoptions_decoupling(dimensions_strict.references, objectiveoptions.decouplingcontrol);
	else
		objectiveoptions.decouplingcontrol = checkobjectiveoptions_decoupling(dimensions_strict.references);
	end
	preventNaN = NaN;
	...
	objective_options_strict = struct(...
		...
		'decouplingcontrol',	struct(...
			'tf_structure',					double(objectiveoptions.decouplingcontrol.tf_structure),...
			'decouplingstrategy',			objectiveoptions.decouplingcontrol.decouplingstrategy,...
			'sortingstrategy_decoupling',	objectiveoptions.decouplingcontrol.sortingstrategy_decoupling,...
			'weight_decoupling',			double(objectiveoptions.decouplingcontrol.weight_decoupling),...
			'weight_prefilter',				double(objectiveoptions.decouplingcontrol.weight_prefilter),...
			'tolerance_decoupling',			double(objectiveoptions.decouplingcontrol.tolerance_decoupling),...
			'tolerance_prefilter',			double(objectiveoptions.decouplingcontrol.tolerance_prefilter),...
			'solvesymbolic',				logical(objectiveoptions.decouplingcontrol.solvesymbolic),...
			'round_equations_to_digits',	double(objectiveoptions.decouplingcontrol.round_equations_to_digits),...
			'allowoutputdecoupling',		logical(objectiveoptions.decouplingcontrol.allowoutputdecoupling)...
		),...
		'objective',			struct(...
