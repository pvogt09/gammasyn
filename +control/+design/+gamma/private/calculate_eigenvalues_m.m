		...
		elseif needsgradient
			...
			if allRNaN
				...
				if needseigenvectors
					...
					if eigenvaluederivativetype == GammaEigenvalueDerivativeType.VANDERAA
						[Gamma, invGamma] = vanDerAa_normalizeEigenvectorMatrix(eigenvectors_right, eigenvectors_left');
						eigenvectors_right = eigenvectors_right*Gamma;
						eigenvectors_left = eigenvectors_left*invGamma';
					end
					if needseigenvectorsright
						eigenvector_right(:, :, ii) = [
							eigenvectors_right,	(1 + 1i)*NaN(system_order, number_states - system_order);
