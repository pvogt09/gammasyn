function [RKF_fixed, RKF_bounds, valid, message] = coupling_RKF_fixed(systems, objectiveoptions, solveroptions, descriptor)
	...
	if control_design_type == GammaCouplingStrategy.APPROXIMATE_INEQUALITY
		R_bounds = convert_vectorized2hadamard([
			X_R;
			-X_R
		], [
			z_R + objectiveoptions.couplingcontrol.tolerance_coupling;
			-z_R - objectiveoptions.couplingcontrol.tolerance_coupling
		], [number_controls, number_states]);
		R_fixed = {[], []};
	else
		if ~isempty(X_R)
			if solvesymbolic
	...
	if control_design_type == GammaCouplingStrategy.APPROXIMATE_INEQUALITY
		F_bounds = convert_vectorized2hadamard([
			...
			z_F + objectiveoptions.couplingcontrol.tolerance_prefilter;
			-z_F - objectiveoptions.couplingcontrol.tolerance_prefilter
		], [number_controls, dim_F1_2]);
		F_bounds{1} = cat(2, F_bounds{1}, zeros(number_controls, dim_F2_2, size(F_bounds{1}, 3)));
		F_fixed_A = [];
		F_fixed_B = [];
	else
		if ~isempty(X_F)
