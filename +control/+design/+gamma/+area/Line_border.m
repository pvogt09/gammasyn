function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = Line_border(re, im, parameter)
	%LINE_BORDER return border of polearea with f < 0 for points left of the border, f = 0 for points on the border and f > 0 for points right of the border
	%	Input:
	...
	if isnan(parameter.line_a) || isnan(parameter.line_b)
		error('control:design:gamma:area:border', 'Invalid parameter structure supplied');
	end
	f = parameter.line_a*(re - parameter.reshift) + parameter.line_b - (im - parameter.imshift);
	if nargout >= 2
		dfdre = parameter.line_a;
		if nargout >= 3
			dfdim = -1;
			if nargout >= 4
				d2fdredre = 0;
