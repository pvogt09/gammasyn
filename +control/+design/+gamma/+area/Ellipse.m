classdef Ellipse < control.design.gamma.area.GammaArea
	%ELLIPSE class for representation of an ellipsoidal pole area sqrt(re^2/a^2 + im^2/b^2) - 1 = 0
	...
	methods(Static=true)
		function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = border(re, im, parameter)
			...
			if nargout >= 7
				[f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			elseif nargout >= 6
				[f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			elseif nargout >= 5
				[f, dfdre, dfdim, d2fdredre, d2fdimdre] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			elseif nargout >= 4
				[f, dfdre, dfdim, d2fdredre] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			elseif nargout >= 3
				[f, dfdre, dfdim] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			elseif nargout >= 2
				[f, dfdre] = control.design.gamma.area.Ellipse_border(re, im, parameter);
			else
				f = control.design.gamma.area.Ellipse_border(re, im, parameter);
			end
		end
		...
		function [dfdre, dfdim] = gradborder(re, im, parameter)
			...
			%		dfdre:		gradient of border function value at point [re, im] for coordinate re
			%		dfdim:		gradient of border function value at point [re, im] for coordinate im
			[~, dfdre, dfdim] = control.design.gamma.area.Ellipse.border(re, im, parameter);
		end
		...
		function [d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = hessborder(re, im, parameter)
			...
			%		d2fdredim:	partial derivative of the gradient dfdim of border function value at [re, im] for coordinate re
			%		d2fdimdim:	partial derivative of the gradient dfdim of border function value at [re, im] for coordinate im
			[~, ~, ~, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = control.design.gamma.area.Ellipse.border(re, im, parameter);
		end
	end
	...
	methods
		function [this] = Ellipse(a, b, varargin)
			%ELLIPSE return new ellipse pole area with specified semi major and minor axes and shift
			%	Input:
			...
			this@control.design.gamma.area.GammaArea(GammaArea.ELLIPSE, shift);
			this.a = a;
			this.b = b;
		end
	...
	methods(Access=protected)
		...
		function [str] = getinstancestring(this, format)
			...
			str = ['Ellipse(', sprintf(format, this.a), ', ', sprintf(format, this.b), this.printshift(this.reshift, this.imshift, format, true), ')'];
		end
