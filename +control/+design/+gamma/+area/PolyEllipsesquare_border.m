function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = PolyEllipsesquare_border(re, im, parameter)
	%POLYELLIPSESQUARE_BORDER return border of polearea with f < 0 for points left of the border, f = 0 for points on the border and f > 0 for points right of the border
	%	Input:
	...
	f = sum((re - parameter.reshift - real(parameter.polyellipse_f)).^2.*parameter.polyellipse_w + (im - parameter.imshift - imag(parameter.polyellipse_f)).^2.*parameter.polyellipse_w) - 1;
	if nargout >= 2
		dfdre = sum(2*(re - parameter.reshift - real(parameter.polyellipse_f)).*parameter.polyellipse_w);
		if nargout >= 3
			dfdim = sum(2*(im - parameter.imshift - imag(parameter.polyellipse_f)).*parameter.polyellipse_w);
			if nargout >= 4
				d2fdredre = sum(parameter.polyellipse_w);
				if nargout >= 5
					d2fdimdre = 0;
					if nargout >= 6
						d2fdredim = 0;
						if nargout >= 7
							d2fdimdim = sum(parameter.polyellipse_w);
						end
					end
				end
			end
