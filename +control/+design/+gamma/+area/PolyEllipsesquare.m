classdef PolyEllipsesquare < control.design.gamma.area.GammaArea
	%POLYELLIPSESQUARE class for representation of a polyellipsoidal pole area w_1(re - re_1)^2 + w_1(im - im_1)^2 + ... + w_n(re - re_n)^2 + w_n(im - im_n)^2 - 1 = 0
	...
	methods(Static=true)
		function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = border(re, im, parameter)
			...
			if nargout >= 7
				[f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			elseif nargout >= 6
				[f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			elseif nargout >= 5
				[f, dfdre, dfdim, d2fdredre, d2fdimdre] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			elseif nargout >= 4
				[f, dfdre, dfdim, d2fdredre] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			elseif nargout >= 3
				[f, dfdre, dfdim] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			elseif nargout >= 2
				[f, dfdre] = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			else
				f = control.design.gamma.area.PolyEllipsesquare_border(re, im, parameter);
			end
		end
		...
		function [dfdre, dfdim] = gradborder(re, im, parameter)
			...
			%		dfdre:		gradient of border function value at point [re, im] for coordinate re
			%		dfdim:		gradient of border function value at point [re, im] for coordinate im
			[~, dfdre, dfdim] = control.design.gamma.area.PolyEllipsesquare.border(re, im, parameter);
		end
		...
		function [d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = hessborder(re, im, parameter)
			...
			%		d2fdredim:	partial derivative of the gradient dfdim of border function value at [re, im] for coordinate re
			%		d2fdimdim:	partial derivative of the gradient dfdim of border function value at [re, im] for coordinate im
			[~, ~, ~, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = control.design.gamma.area.PolyEllipsesquare.border(re, im, parameter);
		end
	end
	...
	methods
		function [this] = PolyEllipsesquare(f, w, varargin)
			%POLYELLIPSESQUARE return new polyellipse pole area with specified weights and focal points and shift
			%	Input:
			%		f:		focal points
			...
			this@control.design.gamma.area.GammaArea(GammaArea.POLYELLIPSESQUARE, shift);
	...
	methods(Access=protected)
		...
		function [str] = getinstancestring(this, format)
			...
			str = ['PolyEllipsesquare([', strjoin(fformated, ', '), '], [', strjoin(wformated, ', '), ']', this.printshift(this.reshift, this.imshift, format, true), ')'];
		end
