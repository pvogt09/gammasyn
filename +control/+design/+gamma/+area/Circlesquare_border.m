function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = Circlesquare_border(re, im, parameter)
	%CIRCLESQUARE_BORDER return border of polearea with f < 0 for points left of the border, f = 0 for points on the border and f > 0 for points right of the border
	%	Input:
	...
	f = (re - parameter.reshift)^2 + (im - parameter.imshift)^2 - parameter.circle_R^2;
	if nargout >= 2
