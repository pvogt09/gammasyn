function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = CircleDiscrete_border(re, im, parameter)
	%CIRCLEDISCRETE_BORDER return border of polearea with f < 0 for points left of the border, f = 0 for points on the border and f > 0 for points right of the border
	%	Input:
	...
	if isnan(parameter.circlediscrete_R)
		error('control:design:gamma:area:border', 'Invalid parameter structure supplied.');
	end
	f = (re - parameter.reshift)^2 + (im - parameter.imshift)^2 - parameter.circlediscrete_R^2;
	if nargout >= 2
		dfdre = 2*(re - parameter.reshift);
		if nargout >= 3
			dfdim = 2*(im - parameter.imshift);
			if nargout >= 4
				d2fdredre = 2;
				if nargout >= 5
					d2fdimdre = 0;
