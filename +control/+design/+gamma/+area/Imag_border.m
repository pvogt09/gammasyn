function [f, dfdre, dfdim, d2fdredre, d2fdimdre, d2fdredim, d2fdimdim] = Imag_border(re, ~, parameter)
	%IMAG_BORDER return border of polearea with f < 0 for points left of the border, f = 0 for points on the border and f > 0 for points right of the border
	%	Input:
	...
	f = parameter.line_a*(re - parameter.reshift) + parameter.line_b;
	if nargout >= 2
