function [J, gradJ, hessianJ] = J(x, system, weight, areafun, dimensions, options)
	%J calculate objective function, gradient and hessian for gamma pole placement
	%This function is functionally equivalent to J_mex_* except, that it supports function handles in areafun, because it does not get compiled
	%	Input:
	...
	if nargout >= 3
		...
		elseif any(options.type == GammaJType.EIGENVALUECONDITION)
			...
			if derivative_feedback
				[eigenvalues, eigenvector_right, eigenvector_left, eigenvalue_derivative, eigenvalue_derivative_xdot, eigenvector_right_derivative, eigenvector_right_derivative_xdot, eigenvector_left_derivative, eigenvector_left_derivative_xdot] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
				...
				[eigenvalues, eigenvector_right, eigenvector_left, eigenvalue_derivative, ~, eigenvector_right_derivative, eigenvector_right_derivative_xdot, eigenvector_left_derivative, eigenvector_left_derivative_xdot] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
			...
			if derivative_feedback
				...
				if any(needseigenvalues(:))
					[eigenvalues, ~, ~, eigenvalue_derivative, eigenvalue_derivative_xdot, ~, ~, ~, ~, eigenvalue_2derivative, eigenvalue_2derivative_xdot, eigenvalue_2derivative_m, eigenvalue_2derivative_xdot_m] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
				...
				needseigenvalues = GammaJType_needseigenvalues(options.type);
				if any(needseigenvalues(:))
					[eigenvalues, ~, ~, eigenvalue_derivative, ~, ~, ~, ~, ~, eigenvalue_2derivative] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
	...
	elseif nargout >= 2
		...
		elseif any(options.type == GammaJType.EIGENVALUECONDITION)
			...
			if derivative_feedback
				...
				needseigenvalues = GammaJType_needseigenvalues(options.type);
				if any(needseigenvalues(:))
					[eigenvalues, ~, ~, eigenvalue_derivative, eigenvalue_derivative_xdot] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
				...
				needseigenvalues = GammaJType_needseigenvalues(options.type);
				if any(needseigenvalues(:))
					[eigenvalues, ~, ~, eigenvalue_derivative] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
		...
		if all(options.type == GammaJType.ZERO)
			J = 0;
		elseif any(options.type == GammaJType.EIGENVALUECONDITION)
			[eigenvalues, eigenvector_right, eigenvector_left] = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
			...
			needseigenvalues = GammaJType_needseigenvalues(options.type);
			if any(needseigenvalues(:))
				eigenvalues = calculate_eigenvalues(system, R, K, dimensions, eigenvaluederivativetype, options.usecompiled, numthreads);
