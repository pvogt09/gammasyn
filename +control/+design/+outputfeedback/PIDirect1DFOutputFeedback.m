classdef PIDirect1DFOutputFeedback < control.design.outputfeedback.OutputFeedback
	%PIDIRECT1DFOUTPUTFEEDBACK class for casting a control system in output feedback form without -RCx and specify the needed constraints on the resulting gain matrix for a PI output feedback controller
	%	For the control system
	%		Ex' = Ax + Bu
	%		y = Cx + 0u
	%		y' = C_dot x
	%	the control law u = K_I x_I + Fw - F C x - F 0 u and the additional dynamics x_I' = 0x_I - Cx - 0u + w results in the output feedback form
	%		[
	%			E,	0
	%			0,	I
	%		]x' = [
	...
	%			I
	%		]w
	%	In contrast to the PIDirectOutputFeedback no -RCx is fed back.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PIDirect1DFOutputFeedback';
		end
	end

	methods
		function [this] = PIDirect1DFOutputFeedback(varargin) %#ok<VANUS> varargin is not used but allowes to call the constructor with arguments
			%PIDIRECT1DFOUTPUTFEEDBACK create new PI output feedback class
			%	Input:
	...
	methods(Access=protected)
		...
		function [R_gain, K_gain, F_prefilter] = gainpattern_parametric_system(~, ~, ~, B, C, ~, ~, ~, ~, ~)
			...
			%		D:				throughput matrix
			%		C_ref:			measurement matrix for reference outputs
			%		K_ref:			throughput matrix for reference outputs
			%		T:				sampling time
			%	Output:
		...
		function [T_x, T_u, T_y, T_y_dot, T_w] = scalegain_system(~, T_x, T_u, T_y, T_y_dot, T_w, ~, ~, B, C, ~, ~, ~, ~, ~)
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, usesCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				C_dot = zeros(0, q);
				D = [
					-partitionR.F,	partitionF.F,	zeros(p, q)
				];
				...
				C_dot = zeros(0, q);
				D = [
					-partitionR.F,	zeros(p, q_dot),	partitionF.F,	zeros(p, q_dot)
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
