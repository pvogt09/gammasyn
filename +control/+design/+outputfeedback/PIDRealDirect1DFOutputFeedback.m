classdef PIDRealDirect1DFOutputFeedback < control.design.outputfeedback.AbstractDRealFeedback
	%PIDREALDIRECT1DFOUTPUTFEEDBACK class for casting a control system in real PID output feedback form without -RCx and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = K_I x_I + K w' - K x_D' + Fw - FCx - FDu - K0u' with the additional dynamic x_I' = 0x_I + w - Cx - Du and x_D' = -T x_D + C_dot x results in the output feedback form
	%		([
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = K_I x_I_k + F w_k - F C x_k - F 0 u_k + K w_k - K x_T_k + K x_D_k - K w_{k-1} with the additional dynamic x_I_{k+1} = x_I_k + w_k - C x_k - D x_k, x_T_{k+1} = -T x_T_k + Cx_k + D u_k and x_D_{k+1} = C x_k + D u_k results in the output feedback form
	%		[
	%			E,	0,	0,	0;
	%			0,	I,	0,	0;
	%			0,	0,	I,	0;
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PIDRealDirect1DFOutputFeedback';
		end
	end
	
	methods
		function [this] = PIDRealDirect1DFOutputFeedback(varargin)
			%PIDREALDIRECT1DFOUTPUTFEEDBACK create new real PID output feedback class
			%	Input:
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				if nargout >= 2
					...
					if nargout >= 3
						...
						if nargout >= 4
							R = cat(1,...
								cat(2, reshape(eye(p*q), p, q, p*q), zeros(p, q + q_dot, p*q)),...
								zeros(q + q_dot, 2*q + q_dot, p*q)...
							);
							...
							RKF_fixed = {
								cat(3,...
									...
									cat(2, zeros(p + q + q_dot, 2*q + q_dot, p*q_dot), K, F_2)...
								), zeros(p*(q + q_dot), 1)
							};
						end
					end
						...
						if nargout >= 8
							...
							if nargout >= 9
								if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
									...
									R_nonlin = constraintfun(false, p + q + 1:q_dot, 2*q + 1:q_dot);
								end
							end
						end
					end
...
function [fun] = constraintfun(isdiscrete, idxR1, idxR2, ~, ~)
	%CONSTRAINTFUN return constraint function for dynamical part of PIDRealDirect1DFOutputFeedback
	%	Input:
