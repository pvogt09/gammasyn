classdef PIObserverDirectOutputFeedback < control.design.outputfeedback.AbstractReferenceModelFeedback
	%PIOBSERVERDIRECTOUTPUTFEEDBACK class for casting a control system in output feedback form and specify the needed constraints on the resulting gain matrix for a PI observer output feedback controller
	%	For the control system
	%		Ex' = Ax + Bu
	%		y = Cx + 0u
	%		y' = C_dot x
	%	the control law u = -R C x_O - R 0 u + K_I x_I + Fw - F C x - F 0 u and the additional dynamics x_I' = 0x_I - Cx - 0u + w and the observer dynamics E_O x_O' = A_O x_O + B_O u + LCx - L C_O x_O results in the output feedback form
	%		[
	%			E,	0,		0;
	%			0,	E_O,	0;
	%			0,	0,		I
	...
	%			I
	%		]w
	%	In contrast to the PIObserverOutputFeedback F (w - y) is fed back directly.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PIObserverDirectOutputFeedback';
		end
	end

	methods
		function [this] = PIObserverDirectOutputFeedback(system, varargin)
			%PIOBSERVERDIRECTOUTPUTFEEDBACK create new PI observer output feedback class
			%	Input:
	...
	methods(Access=protected)
		function [E, A, B, C, C_dot, D, C_ref, D_ref] = amend_system(this, E, A, B, C, ~, D, C_ref, ~, T)
			...
			C_ref = [
				C_ref, zeros(size(C_ref, 1), size(Enom, 1) + q)
			];
		...
		function [R_fixed, K_fixed, F_fixed, RKF_fixed, R_bounds, K_bounds, F_bounds, RKF_bounds, R_nonlin] = gainpattern_system(this, ~, ~, B, C, ~, ~, ~, ~, ~)
			%GAINPATTERN_SYSTEM return gain pattern constraint system for a PI observer output feedback gain matrix with gain matrix K = [
			%		F,	R,	-K_I;
			%		-L,	L,	0;
			%		I,	0,	0
			%	];
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, usesCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				B = [
					...
					partitionR.L - this.B_model_ref*partitionR.F,		this.B_model_ref*partitionF.F,	zeros(nnom, q)
				];
				C = [
				...
				D = [
					-partitionF.F,	partitionF.F,	zeros(p, q)
				];
				...
				C_dot = zeros(0, q + nnom);
				D = [
					-partitionF.F,	zeros(p, q_dot),	partitionF.F,	zeros(p, q_dot)
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
