classdef PDRealDirect1DFOutputFeedback < control.design.outputfeedback.AbstractDRealFeedback
	%PDREALDIRECT1DFOUTPUTFEEDBACK class for casting a control system in real PD output feedback form without RCx and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = -FCx + K x_D' + Fw + K w' with the additional dynamic x_D' = -T x_D + C_dot x results in the output feedback form
	%		[
	%			E,	0;
	%			0,	I
	%		][
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = -FCx_k - F 0 u_k + K x_D_k - K x_T_k + Fw_k + Kw_k + K w_{k-1} with the additional dynamic x_T_{k+1} = -T x_T_k + Cx_k + D_u_k and  x_D_{k+1} = x_T_k results in the output feedback form
	%		[
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	%			w_{k-1}
	%		]
	%	In contrast to the PDRealDirectOutputFeedback no -RCx is fed back.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PDRealDirect1DFOutputFeedback';
		end
	end

	methods
		function [this] = PDRealDirect1DFOutputFeedback(varargin)
			%PDREALDIRECT1DFOUTPUTFEEDBACK create new real PD output feedback class
			%	Input:
	...
	methods(Access=protected)
				...
				if nargout >= 2
					...
					if nargout >= 3
						...
						if nargout >= 4
							...
							K = cat(1,...
								cat(2, zeros(p, 2*q, p*q), reshape(eye(p*q), p, q, p*q)),...
								zeros(p + q, 3*q, p*q)...
							);
							F_1 = cat(1,...
				...
				if nargout >= 2
					...
					if nargout >= 3
						F_fixed = {[
							...
							true(q_dot, q),	false(q_dot, q_dot)
						], [
							NaN(p, q),			zeros(p, q_dot);
							zeros(q_dot, q),	NaN(q_dot, q_dot)
						]};
						if nargout >= 4
							R = cat(1,...
								cat(2, reshape(eye(p*q), p, q, p*q), zeros(p, q_dot, p*q)),...
								zeros(q_dot, q + q_dot, p*q)...
							);
							K = cat(1,...
								reshape(eye(p*q_dot), p, q_dot, p*q_dot),...
								zeros(q_dot, q_dot, p*q_dot)...
							);
							F_1 = cat(1,...
								cat(2, reshape(-eye(p*q), p, q, p*q), zeros(p, q_dot, p*q)),...
								zeros(q_dot, q + q_dot, p*q)...
							...
							F_2 = cat(1,...
								...
								zeros(q_dot, q + q_dot, p*q_dot)...
							);
							RKF_fixed = {
								cat(3,...
									cat(2, R, zeros(p + q_dot, q_dot, p*q), F_1),...
									cat(2, zeros(p + q_dot, q + q_dot, p*q_dot), K, F_2)...
								), zeros(p*(q + q_dot), 1)
							};
						end
					end
		...
		function [F, F_fixed] = prefilterpattern_system(~, R, K, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				F = [
					R(1:p, 1:q),		zeros(p, q_dot);
					zeros(q_dot, q),	-K(1:q_dot, 1:q_dot)
				];
				if nargout >= 2
					F_fixed = true(p + q_dot, q + q_dot);
				end
			end
		end
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, useCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			%		D_ref:			throughput matrix for reference outputs of contoller
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				C_dot = zeros(0, 2*q);
				D = [
					-partitionR.F,	partitionF.F + partitionF.K_D,	partitionF.K_D
				];
				...
				C_dot = zeros(0, q_dot);
				D = [
					-partitionR.F,	zeros(p, q_dot),	partitionF.F,	partitionF.K_D
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
...
function [fun] = constraintfun(isdiscrete, idxR1, idxR2, ~, ~)
	%CONSTRAINTFUN return constraint function for dynamical part of PDRealDirect1DFOutputFeedback
	%	Input:
