classdef PIDDirect1DFOutputFeedback < control.design.outputfeedback.OutputFeedback
	%PIDDIRECT1DFOUTPUTFEEDBACK class for casting a control system in PID output feedback form without RCx and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = K_I x_I + K w' - K x_D' + Fw - FCx - FDu - K0u' with the additional dynamic x_I' = 0x_I + w - Cx - Du results in the output feedback form
	%		([
	%			E,	0;
	%			0,	I
	%		] - [
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = K_I x_I_k + F w_k - F C x_k - F 0 u_k + K w_k - K C x_k + K x_D_k - K w_{k-1} with the additional dynamic x_I_{k+1} = x_I_k + w_k - C x_k - D x_k and x_D_{k+1} = C x_k + D u_k results in the output feedback form
	%		[
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	%			w_{k-1}
	%		]
	%	In contrast to the PIDDirectOutputFeedback no -RCx is fed back.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PIDDirect1DFOutputFeedback';
		end
	end

	methods
		function [this] = PIDDirect1DFOutputFeedback(varargin) %#ok<VANUS> varargin is not used but allowes to call the constructor with arguments
			%PIDDIRECT1DFOUTPUTFEEDBACK create new PID output feedback class
			%	Input:
	...
	methods(Access=protected)
						...
						if nargout >= 4
							...
							RKF_fixed = {
								cat(3,...
									...
									cat(2, zeros(p + q, 2*q, p*q_dot), K, F_2)...
								), zeros(p*(q + q_dot), 1)
							};
						end
					end
		...
		function [F, F_fixed] = prefilterpattern_system(~, R, K, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				F = [
					R(1:p, 1:q),			R(1:p, 2*q + 1:3*q);
					eye(q),					zeros(q, q);
					-R(1:p, 2*q + 1:3*q),	zeros(p, q)
				];
				if nargout >= 2
					F_fixed = [
						false(p, q),	true(p, q);
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, useCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				C_dot = zeros(0, 2*q);
				D = [
					-(partitionR.F + partitionR.K_D),	partitionF.F + partitionF.K_D,	-partitionF.K_D
				];
				...
				C_dot = zeros(0, q);
				D = [
					-partitionR.F,	partitionR.K_D,	partitionF.F,	partitionF.K_D
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
