classdef PIDOutputFeedback < control.design.outputfeedback.OutputFeedback
	%PIDOUTPUTFEEDBACK class for casting a control system in PID output feedback form and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = -RCx + K_I x_I + K C_dot x' + Fw + F_D w' with the additional dynamic x_I' = 0x_I + w - Cx -Du results in the output feedback form
	%		([
	%			E,	0;
	%			0,	I
	%		] - [
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = -RCx_k + K_I x_I_k + K C x_k - K x_D_k + F w_k + K w_{k-1} with the additional dynamic x_I_{k+1} = x_I_k + w_k - C x_k - D x_k and x_D_{k+1} = C x_k + D u_k results in the output feedback form
	%		[
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	methods(Access=protected)
		...
		function [R_fixed, K_fixed, F_fixed, RKF_fixed, R_bounds, K_bounds, F_bounds, RKF_bounds, R_nonlin] = gainpattern_system(~, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				R_fixed = {~isnan(R), R};
				if nargout >= 2
					K_fixed = {true(p + 2*q, 0), zeros(p + 2*q, 0)};
					if nargout >= 3
						F_fixed = {[
							true(p, q),		false(p, q);
							true(2*q, 2*q)
						], [
							eye(p, q),	R(1:p, 2*q + 1:3*q);
							eye(q),		zeros(q, q);
							zeros(q, 2*q)
						]};
						if nargout >= 4
							R = cat(1, cat(2, zeros(p, 2*q, p*q), reshape(eye(p*q), p, q, p*q)), zeros(2*q, 3*q, p*q));
							F = cat(1, cat(2, zeros(p, q, p*q), reshape(-eye(p*q), p, q, p*q)), zeros(2*q, 2*q, p*q));
							RKF_fixed = {
								cat(2, R, zeros(p + 2*q, 0, p*q), F), zeros(p*q, 1)
							};
						end
					end
				end
		...
		function [F, F_fixed] = prefilterpattern_system(~, R, ~, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				if nargout >= 2
					F_fixed = [
						false(p, q),	true(p, q);% TODO: does not match elements of F
						true(2*q, 2*q)
					];
				end
			else
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, usesCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				C_dot = zeros(0, 2*q);
				D = [
					-(partitionR.R - partitionR.K_D),	partitionF.F,	partitionF.K_D
				];
