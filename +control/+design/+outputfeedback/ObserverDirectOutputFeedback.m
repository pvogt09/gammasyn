classdef ObserverDirectOutputFeedback < control.design.outputfeedback.AbstractReferenceModelFeedback
	%OBSERVERDIRECTOUTPUTFEEDBACK class for casting a control system in output feedback form and specify the needed constraints on the resulting gain matrix for a observer output feedback controller
	%	For the control system
	%		Ex' = Ax + Bu
	%		y = Cx + 0u
	%	the control law u = -R C x_O - R 0 u + Fw - F C x - F 0 u and the observer dynamics E_O x_O' = A_O x_O + B_O u + LCx - L C_O x_O results in the output feedback form
	%		[
	%			E,	0;
	%			0,	E_O
	%		]x' = [
	...
	methods(Access=protected)
		...
		function [R_fixed, K_fixed, F_fixed, RKF_fixed, R_bounds, K_bounds, F_bounds, RKF_bounds, R_nonlin] = gainpattern_system(this, ~, ~, B, C, ~, ~, ~, ~, ~)
			...
			R_fixed_system = NaN([size(R), number_observer]);
			R_fixed_border = zeros(number_observer, 1);
			[row, col] = find(R == 1);
			...
			if nargout >= 3
				...
				if nargout >= 4
					...
					RKF_fixed = {
						cat(2, R, zeros(p + nnom, 0, p*q), F), zeros(p*q, 1)
					};
				end
			end
			if nargout >= 5
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, useCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				B = [
					-this.B_model_ref*(partitionR.F + partitionR.R) + partitionR.L,	this.B_model_ref*partitionF.F,	zeros(nnom, q)
				];
				C = -(partitionR.F + partitionR.R)*this.C_model_ref;
				C_dot = zeros(0, nnom);
				D = [
				...
				A = this.A_model_ref - partitionR.L*this.C_model_ref;
				B = [
					-this.B_model_ref*(partitionR.F + partitionR.R) + partitionR.L, zeros(nnom, q_dot),	this.B_model_ref*partitionF.F,	zeros(nnom, q_dot)
				];
				C = -(partitionR.F + partitionR.R)*this.C_model_ref;
				C_dot = zeros(0, nnom);
				D = [
