classdef PIDDynamicOutputFeedback < control.design.outputfeedback.AbstractDynamicOrderModelFeedback
	%PIDDYNAMICOUTPUTFEEDBACK class for casting a control system in PID output feedback form with additional dynamics and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = -RCx + K_I x_I + K_w w' - K C_dot x' + Fw + C_D x_D + D_D C x + D_D D u with the additional dynamic x_I' = 0x_I + w - Cx - Du and x_D' = A_D x_D + B_D C x + B_D D u results in the output feedback form
	%		([
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = -RCx_k + K_I x_I_k + F w_k + K w_k - K C x_k + K x_D_k - K w_{k-1} + C_D x_A_k + D_D C x_k + D_D 0 u_k with the additional dynamic x_I_{k+1} = x_I_k + w_k - C x_k - D x_k, x_D_{k+1} = C x_k + D u_k and x_A_{k+1} = A_D x_k + B_D C x_k + B_D D u_k results in the output feedback form
	%		[
	%			E,	0,	0,	0;
	%			0,	I,	0,	0;
	%			0,	0,	I,	0;
	...
	%			w_{k-1}
	%		]
	%	In contrast to the PIDDirectDynamicOutputFeedback F (w - y) is not fed back directly.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PIDDynamicOutputFeedback';
		end
	end

	methods
		function [this] = PIDDynamicOutputFeedback(system, varargin)
			%PIDDYNAMICOUTPUTFEEDBACK create new PID output feedback class with additional dynamics
			%	Input:
	...
	methods(Access=protected)
		function [E, A, B, C, C_dot, D, C_ref, D_ref] = amend_system(this, E, A, B, C, C_dot, D, C_ref, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				B = [
					B,	zeros(n, q),	zeros(n, nnom);
					-D,	eye(q),			zeros(q, nnom);
					D,	zeros(q, q),	zeros(q, nnom);
		...
		function [R_fixed, K_fixed, F_fixed, RKF_fixed, R_bounds, K_bounds, F_bounds, RKF_bounds, R_nonlin] = gainpattern_system(this, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				R = [
					NaN(p, q),		D_nom(1:p, 1:q),	NaN(p, 2*q),		C_nom(1:p, 1:nnom);
					zeros(q, 4*q + nnom);
					zeros(nnom, q),	B_nom(1:nnom, 1:q),	zeros(nnom, 2*q),	A_nom(1:nnom, 1:nnom)
				];
				R_fixed = {~isnan(R), R};
				if nargout >= 2
					K_fixed = {true(p + q + nnom, 0), zeros(p + q + nnom, 0)};
					if nargout >= 3
						F_fixed = {[
							false(p, 2*q);
							true(q + nnom, 2*q)
						...
						if nargout >= 4
							...
							K = cat(1,...
								cat(2, zeros(p, 3*q, p*q), reshape(eye(p*q), p, q, p*q), zeros(p, nnom, p*q)),...
								zeros(q + nnom, 4*q + nnom, p*q)...
							);
							F_1 = cat(1,...
							...
							RKF_fixed = {
								cat(3,...
									cat(2, R, zeros(p + q + nnom, 0, p*q), F_1),...
									cat(2, K, zeros(p + q + nnom, 0, p*q), F_2)...
								), zeros(2*p*q, 1)
							};
						end
					end
				...
				R = [
					NaN(p, q),		D_nom(1:p, 1:q),	NaN(p, q),		C_nom(1:p, 1:nnom);
					zeros(q, 3*q + nnom);
					zeros(nnom, q),	B_nom(1:nnom, 1:q),	zeros(nnom, q),	A_nom(1:nnom, 1:nnom)
				];
				R_fixed = {~isnan(R), R};
				if nargout >= 2
					K = [
					...
					if nargout >= 3
						...
						if nargout >= 4
							R = cat(1,...
								cat(2, reshape(eye(p*q), p, q, p*q), zeros(p, 2*q + nnom, p*q)),...
								zeros(q + nnom, 3*q + nnom, p*q)...
							);
							F = cat(1,...
							...
							RKF_fixed = {
								cat(2, R, zeros(p + q + nnom, q_dot, p*q), F), zeros(p*q, 1)
							};
						end
					end
				end
				...
				if nargout >= 6
					...
					if nargout >= 7
						 ...
						 if nargout >= 8
							...
							if nargout >= 9
								if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
									R_nonlin = constraintfun(true, p + q + (1:nnom), 4*q + (1:nnom));
								else
		...
		function [R_gain, K_gain, F_prefilter] = gainpattern_parametric_system(this, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				R_gain = [
					R,				-D_D,	-K_I,	-K,			-C_D;
					zeros(q, 4*q + nnom);
					zeros(nnom, q),	-B_D,	zeros(nnom, 2*q),	-A_D
				];
				if nargout >= 2
					K_gain = zeros(p + q + nnom, 0);
					if nargout >= 3
						F_prefilter = [
							F,		K;
							eye(q),	zeros(q, q);
				...
				R_gain = [
					R,			-D_D,	-K_I,			-C_D;
					zeros(q, 3*q + nnom);
					zeros(nnom, q),	-B_D,	zeros(nnom, q),	-A_D
				];
				if nargout >= 2
					K_gain = [
						-K;
		...
		function [T_x, T_u, T_y, T_y_dot, T_w] = scalegain_system(this, T_x, T_u, T_y, T_y_dot, T_w, ~, ~, ~, C, ~, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				if nargout >= 2
					...
					if nargout >= 3
						T_y = blkdiag(T_y, T_y, eye(q), T_y, eye(nnom));
						if nargout >= 5
							T_w = blkdiag(T_w, T_w);
						end
					end
				...
				if nargout >= 2
					...
					if nargout >= 3
						...
						T_y = blkdiag(T_y, T_y, eye(q + nnom));
					end
				end
			end
		end
		...
		function [partitionR, partitionF] = gainpartitioning_system(this, R, K, F, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				partitionR = struct(...
					...
					'K_I',	-R(1:p, 2*q + 1:3*q),...
					'K_D',	-K(1:p, 1:q_dot),...
					'C_D',	-R(1:p, 3*q + 1:3*q + nnom),...
					'B_D',	-R(p + q + 1:p + q + nnom, q + 1:2*q),...
					'A_D',	-R(p + q + 1:p + q + nnom, 3*q + 1:3*q + nnom)...
				);
				if nargout >= 2
					partitionF = struct(...
						'F',	F(1:p, 1:q),...
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, usesCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				D = [
					-(partitionR.R + partitionR.K_D - partitionR.D_D),	partitionF.F + partitionF.K_D,	-partitionF.K_D
				];
				...
				D = [
					-(partitionR.R - partitionR.D_D),	-partitionR.K_D,	partitionF.F,	partitionF.K_D
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
...
function [fun] = constraintfun(isdiscrete, idxR1, idxR2, ~, ~)
	%CONSTRAINTFUN return constraint function for dynamical part of PIDDynamicOutputFeedback
	%	Input:
