classdef PDRealDirectOutputFeedback < control.design.outputfeedback.AbstractDRealFeedback
	%PDREALDIRECTOUTPUTFEEDBACK class for casting a control system in real PD output feedback form and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = -RCx - FCx + K x_D' + Fw + K w' with the additional dynamic x_D' = -T x_D + C_dot x results in the output feedback form
	%		[
	%			E,	0;
	%			0,	I
	%		][
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = -RCx_k - FCx_k - F 0 u_k + K x_D_k - K x_T_k + Fw_k + Kw_k + K w_{k-1} with the additional dynamic x_T_{k+1} = -T x_T_k + Cx_k + D_u_k and  x_D_{k+1} = x_T_k results in the output feedback form
	%		[
	%			E,	0,	0;
	%			0,	I,	0;
	%			0,	0,	I
	...
	%			w_{k-1}
	%		]
	%	In contrast to the PDRealOutputFeedback F (w - y) is fed back directly.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PDRealDirectOutputFeedback';
		end
	end

	methods
		function [this] = PDRealDirectOutputFeedback(varargin)
			%PDREALDIRECTOUTPUTFEEDBACK create new real PD output feedback class
			%	Input:
	...
	methods(Access=protected)
		function [E, A, B, C, C_dot, D, C_ref, D_ref] = amend_system(~, E, A, B, C, C_dot, D, C_ref, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				B = [
					B,					zeros(n, q_dot),	B;
					zeros(q_dot, p),	eye(q_dot),			zeros(q_dot, p)
				];
				C = [
					C,					zeros(q, q_dot);
					zeros(q_dot, n),	eye(q_dot)
		...
		function [R_fixed, K_fixed, F_fixed, RKF_fixed, R_bounds, K_bounds, F_bounds, RKF_bounds, R_nonlin] = gainpattern_system(this, ~, A, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				R = [
					NaN(p, q),	NaN(p, q),		zeros(p, q),	NaN(p, q);
					zeros(q, 2*q),				NaN(q, q),		zeros(q, q);
					zeros(p, 4*q)
				];
				nanrealK = ~isnan(realK(1:q, 1:q));
				[nanrow, nancol] = find(nanrealK);
				constrainedrealK = size(nanrow, 1);
				[row, col] = find(R == 0);
				number_zeros = size(row, 1);
				R_fixed_system = NaN([size(R), number_zeros + constrainedrealK]);
				R_fixed_border = zeros(number_zeros + constrainedrealK, 1);
				parfor ii = 1:number_zeros
					temp = zeros(size(R));
					temp(row(ii), col(ii)) = 1;
					R_fixed_system(:, :, ii) = temp;
				end
				parfor ii = 1:constrainedrealK
					temp = zeros(size(R));
				...
				if nargout >= 2
					...
					if nargout >= 3
						...
						if nargout >= 4
							R = cat(1,...
								cat(2, zeros(p, q, p*q), reshape(eye(p*q), p, q, p*q), zeros(p, 2*q, p*q)),...
								zeros(p + q, 4*q, p*q)...
							);
							K = cat(1,...
				...
				if nargout >= 2
					...
					if nargout >= 3
						F_fixed = {[
							false(p, q),	true(p, q_dot);
							true(q_dot, q + q_dot);
							true(p, q),		false(p, q_dot)
						], [
							NaN(p, q),		zeros(p, q_dot);
							zeros(q_dot, q + q_dot);
							zeros(p, q),	NaN(p, q_dot)
						]};
						if nargout >= 4
							R = cat(1,...
								cat(2, reshape(eye(p*q), p, q, p*q), zeros(p, q_dot, p*q)),...
								zeros(q_dot + p, q + q_dot, p*q)...
							);
							K = cat(1,...
								reshape(eye(p*q_dot), p, q_dot, p*q_dot),...
								zeros(q_dot + p, q_dot, p*q_dot)...
							);
							F_1 = cat(1,...
								cat(2, reshape(-eye(p*q), p, q, p*q), zeros(p, q_dot, p*q)),...
								zeros(q_dot + p, q + q_dot, p*q)...
							...
							F_2 = cat(1,...
								zeros(p + q_dot, q + q_dot, p*q_dot),...
								cat(2, zeros(p, q, p*q_dot), reshape(eye(p*q_dot), p, q_dot, p*q_dot))...
							);
							RKF_fixed = {
								cat(3,...
									cat(2, R, zeros(2*p + q_dot, q_dot, p*q), F_1),...
									cat(2, zeros(2*p + q_dot, q + q_dot, p*q_dot), K, F_2)...
								), zeros(p*(q + q_dot), 1)
							};
						end
					end
				...
				if nargout >= 6
					...
					if nargout >= 7
						...
						if nargout >= 8
							...
							if nargout >= 9
								if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
									R_nonlin = constraintfun(true, p + 1:q, 2*q + 1:q);
								else
		...
		function [F, F_fixed] = prefilterpattern_system(~, R, K, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				F = [
					R(1:p, q + 1:2*q),		zeros(p, q);
					zeros(q, 2*q);
					R(1:p, 3*q + 1:4*q),	R(1:p, 3*q + 1:4*q)
				];
				if nargout >= 2
					F_fixed = true(2*p + q, 2*q);
				end
			else
				F = [
					R(p + q_dot + 1:2*p + q_dot, 1:q),		zeros(p, q_dot);
					zeros(q_dot, q + q_dot);
					zeros(p, q),	-K(1:p, 1:q_dot)
				];
				if nargout >= 2
					F_fixed = true(2*p + q_dot, q + q_dot);
				end
			end
		end
		...
		function [partitionR, partitionF] = gainpartitioning_system(~, R, K, F, ~, ~, B, C, C_dot, ~, ~, ~, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				partitionR = struct(...
					...
					'F',	R(1:p, q + 1:2*q),...
					'K_D',	R(1:p, 3*q + 1:4*q),...
					'T',	R(p + 1:p + q, 2*q + 1:3*q)...
				);
				if nargout >= 2
					partitionF = struct(...
						'F',	F(1:p, 1:q),...
						'K_D',	F(p + q + 1:q + 2*p, 1:q)...
					);
				end
			else
				partitionR = struct(...
					'R',	R(1:p, 1:q),...
					'F',	R(p + q_dot + 1:q_dot + 2*p, 1:q),...
					'K_D',	-K(1:p, 1:q_dot),...
		...
		function [E, A, B, C, C_dot, D, C_ref, D_ref, needsstate, useCasCdot] = realization_system(this, R, K, F, E, A, B, C, C_dot, D, C_ref, D_ref, T)
			...
			if control.design.outputfeedback.OutputFeedback.isdiscreteT(T)
				...
				C_dot = zeros(0, 2*q);
				D = [
					-(partitionR.F + partitionR.R),	partitionF.F + partitionF.K_D,	partitionF.K_D
				];
				...
				C_dot = zeros(0, q_dot);
				D = [
					-(partitionR.F + partitionR.R),	zeros(p, q_dot),	partitionF.F,	partitionF.K_D
				];
				C_ref = zeros(0, size(A, 1));
				D_ref = zeros(0, size(B, 2));
			end
...
function [fun] = constraintfun(isdiscrete, idxR1, idxR2, ~, ~)
	%CONSTRAINTFUN return constraint function for dynamical part of PDRealDirectOutputFeedback
	%	Input:
