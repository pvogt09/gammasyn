classdef PDDirect1DFOutputFeedback < control.design.outputfeedback.OutputFeedback
	%PDDIRECT1DFOUTPUTFEEDBACK class for casting a control system in PD output feedback form without -RCx and specify the needed constraints on the resulting gain matrix
	%	For the continuous control system
	%		Ex' = Ax + Bu
	%		y = Cx + Du
	%		y' = C_dot x
	%	the control law u = -FCx - F0u - Kw' + K C_dot x' + K0u' + Fw results in the output feedback form
	%		(E - B K C_dot)x' = Ax - B [
	%			F
	%		][
	%			C
	...
	%		y_k = Cx_k + Du_k
	%		y_{k+1} = C_dot x_{k+1}
	%	the control law u = -FCx_k - KCx_k + K x_D_k + F w_k + K w_k - K w_{k-1} with the additional dynamic x_D_{k+1} = C x_k + D u_k results in the output feedback form
	%		[
	%			E,	0;
	%			0,	I
	%		]x_{k+1} = [
	...
	%			w_{k-1}
	%		]
	%	In contrast to the PDDirectOutputFeedback no -RCx is fed back.
	...
	methods(Static=true)
		function [name] = SimulinkVariant()
			...
			name = 'PDDirect1DFOutputFeedback';
		end
	end

	methods
		function [this] = PDDirect1DFOutputFeedback(varargin) %#ok<VANUS> varargin is not used but allowes to call the constructor with arguments
			%PDDIRECT1DFOUTPUTFEEDBACK create new PD output feedback class
			%	Input:
