classdef PIOutputFeedback < control.design.outputfeedback.OutputFeedback
	%PIOUTPUTFEEDBACK class for casting a control system in output feedback form and specify the needed constraints on the resulting gain matrix for a PI output feedback controller
	%	For the control system
	%		Ex' = Ax + Bu
	%		y = Cx + 0u
	%		y' = C_dot x
	%	the control law u = -R C x_O - R 0 u + K_I x_I + Fw and the additional dynamics x_I' = 0x_I - Cx - 0u + w results in the output feedback form
	%		[
	%			E,	0;
	%			0,	I
	%		]x' = [
