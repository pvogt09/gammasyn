classdef ConstantOutputFeedback < control.design.outputfeedback.OutputFeedback
	%CONSTANTOUTPUTFEEDBACK class for casting a control system in output feedback form and specify the needed constraints on the resulting gain matrix for a constant output feedback controller
	...
	methods(Access=protected)
		function [E, A, B, C, C_dot, D, C_ref, D_ref] = amend_system(~, E, A, B, C, ~, D, C_ref, ~, ~)
			...
			if control.design.outputfeedback.OutputFeedback.isranksupported(E) && rank(E) < size(A, 1)
				error('control:design:outputfeedback:input', 'Descriptor matrix must be regular for a constant output feedback controller.');
			end
